#!/usr/bin/env python3
"""
Main launcher for AI Trading with Performance Optimizer - Clean Version
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if models:
                ...
                logger.info(f"[OK] Ollama connected - Available models: {', '.join(model_names)}")
                return True
            else:
        ...
        logger.error(f"[ERROR] Ollama connection error: {e}")
        return False
...
def main():
    ...
    if success:
        logger.info("[OK] Trading system completed successfully")
        sys.exit(0)
    else:
        logger.error("[ERROR] Trading system failed")
        sys.exit(1)
